...
def nce_criterion(student_feat, teacher_feat, nce_T=0.075, max_samples=8192):
    ...
    nce_logits = torch.mm(student_feat, teacher_feat.transpose(0, 1))                      
...
def lsp_distillation_loss(
    ...
    if similarity_type == 'gaussian':
        ...
        t_diff = torch.abs(teacher_node_emb[src] - teacher_node_emb[dst])          
        t_dist = torch.sum(t_diff, dim=-1)       
        t_score = torch.exp(-t_dist / temperature)       

        s_diff = torch.abs(student_node_emb[src] - student_node_emb[dst])          
        s_dist = torch.sum(s_diff, dim=-1)       
        s_score = torch.exp(-s_dist / temperature)       
    ...
    t_prob = softmax(t_score, index=src, num_nodes=teacher_node_emb.size(0))       

    s_log_prob = F.log_softmax(
        torch.log(s_score + 1e-16).unsqueeze(0),               
        dim=0
    ).squeeze(0)

    s_prob_raw = softmax(s_score, index=src, num_nodes=student_node_emb.size(0))
    s_log_prob = torch.log(s_prob_raw + 1e-16)       
...
def sale_deepwalk_loss(student_node_emb: torch.Tensor, 
    ...
    u = walks[:, :, 0].unsqueeze(2).expand(-1, -1, walk_length)            
    ...
    emb_u = student_node_emb[u]                       
    emb_v_pos = student_node_emb[v_pos]               
